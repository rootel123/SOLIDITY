...
contract QUIZ5 {
    ...
    function register(string memory _name) public {
        user_info[msg.sender] = USER(idx++, _name, msg.sender, 0, 0);
    }
    ...
    function joinGame() public payable {
        require(msg.value == 0.01 ether || user_info[msg.sender].balance >= 0.01 ether);

        if(msg.value==0) {
            user_info[msg.sender].balance -= 0.01 ether;
        }
        ...
        // top4에 들어가기
        top4.push(msg.sender);
    }

    // * 점수를 돈으로 바꾸는 기능 - 10점마다 0.1eth로 환전
    function withdraw(uint _n) public {
        require(_n%10==0 && user_info[msg.sender].score >= _n);
        user_info[msg.sender].score -= 10;
        payable(msg.sender).transfer(_n/10* 0.1 ether);
    }

    // * 점수를 돈으로 바꾸는 기능 - 10점마다 0.1eth로 환전(최대 가능 금액 자동 환전)
    ...
    function ownerWithdrawMax() public onlyOwner{
        owner.transfer(address(this).balance);
    }

    //* 예치 기능 - 게임할 때마다 참가비를 내지 말고 유저가 일정금액을 미리 예치하고 게임할 때마다 자동으로 차감시키는 기능.
    function deposit() public payable {
        user_info[msg.sender].balance += msg.value;
    }
}